  ...
  return (
    <header className="flex flex-row items-center bg-header top-0 w-full h-16 justify-between px-5 md:px-10 lg:px-20 sticky z-50">
      ...
      <nav className="hidden md:flex flex-row gap-6 items-center">
        ...
        {/* Creativity Dropdown */}
        <DropdownMenu>
          <DropdownMenuTrigger className="flex items-center gap-1 font-medium text-header-foreground hover:text-accent transition-colors outline-none">
            Creativity
          ...
          <DropdownMenuContent 
            align="start" 
            sideOffset={16}
            className="bg-header border-header-foreground/20 min-w-[160px] z-50 mt-0"
          >
            {creativityItems.map((item) => (
              <DropdownMenuItem key={item.path} asChild>
                <Link 
                  to={item.path}
                  className="text-header-foreground hover:text-accent hover:bg-header-foreground/10 cursor-pointer text-base py-2"
                >
                  {item.name}
                </Link>
              </DropdownMenuItem>
